...
const AddUnitCategoryModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: newUnitCategory, error: insertError } = await supabase
        .from("unit_categories")
        .insert([{ unit_category: trimmedName, user_id: userProfile.id }])
        .select()
        .single();
      ...
      console.error("Error adding unit category:", err);
      if (err.code === "23505") {
        setError("This unit category name has already been taken.");
      } else {
...
export default AddUnitCategoryModal;
