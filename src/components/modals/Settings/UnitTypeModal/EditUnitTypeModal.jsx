//src/components/modals/UnitTypeModal/EditUnitTypeModal.jsx
...
const EditUnitTypeModal = ({ isOpen, onClose, onSuccess, unit }) => {
  const [unitTypeName, setUnitTypeName] = useState("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState("");

  // When the modal opens, pre-fill the form with the current unit's name
  useEffect(() => {
    if (unit) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!unit || !unit.id) {
      setError("Invalid unit data. Please try again.");
      return;
    }
    ...
    try {
      // Check if another unit type with the same name already exists
      const { data: existing, error: checkError } = await supabase
        .from("unit_types")
        .select("id")
        .ilike("unit_type", trimmedName)
        .neq("id", unit.id) // IMPORTANT: Exclude the current unit from the check
        .single();
      ...
      setError("Failed to update unit type. Please try again.");
    } finally {
      setIsSubmitting(false);
    }
  };
